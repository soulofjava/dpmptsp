...
class Modul_model extends CI_Model
{
  ...
  function get_data($where = array())
	...
	public function json_all_modul($where, $limit, $start, $fields, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
  ...
  function simpan_modul($data_input, $table_name)
	{
		$this->db->insert( $table_name, $data_input );
		return $this->db->insert_id();
	}
		
  function update_data_modul($data_update, $where, $table_name)
	{
		$this->db->where($where);
		$this->db->update($table_name, $data_update);
	}
	
	function json_semua_modul($where) {	
		$this->db->from('modul');
		$this->db->where($where);
	...
	function count_all_search_modul($where, $key_word, $table_name, $field) {	
		$this->db->from($table_name);
		$this->db->where($where);
	...
	public function search_modul($fields, $where, $limit, $start, $key_word, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
		$this->db->like('urut_modul', $key_word);
		$this->db->order_by($order_by);
		$this->db->limit($limit, $start);
		$result = $this->db->get('modul');
		return $result->result_array();
	...
	function auto_suggest($q, $where, $fields) {
		$this->db->select("$fields");
		$this->db->where($where);
		$this->db->like('modul.urut_modul', $q);
		$this->db->or_like('modul.nama_modul', $q);
		$this->db->limit('20');
		$result = $this->db->get('modul');
		return $result->result_array();
	}
