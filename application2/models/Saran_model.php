...
class Saran_model extends CI_Model
{
	...
	public function json_all_saran($where, $limit, $start, $fields, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
  ...
  function simpan_saran($data_input, $table_name)
	{
		$this->db->insert( $table_name, $data_input );
		return $this->db->insert_id();
	}
		
  function update_data_saran($data_update, $where, $table_name)
	{
		$this->db->where($where);
		$this->db->update($table_name, $data_update);
	}
	
	function json_semua_saran($where) {	
		$this->db->from('saran');
		$this->db->where($where);
	...
	function count_all_search_saran($where, $key_word, $table_name, $field) {	
		$this->db->from($table_name);
		$this->db->where($where);
	...
	public function search_saran($fields, $where, $limit, $start, $key_word, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
		$this->db->like('telp_saran', $key_word);
		$this->db->order_by($order_by);
		$this->db->limit($limit, $start);
		$result = $this->db->get('saran');
		return $result->result_array();
	...
	public function json_buku_kategori_buku($where) {
		$this->db->select("
		kategori_buku.kategori_buku_telp,
	...
	public function json_laborat_perbagian($where) {
		...
		bagian_puskesmas.telp_bagian_puskesmas,
		pus_pelayanan.id_pus_pelayanan,
		pasien.telp_penduduk,
	...
	function auto_suggest($q, $where, $fields) {
		$this->db->select("$fields");
		$this->db->where($where);
		$this->db->like('saran.telp_saran', $q);
		$this->db->or_like('saran.nik', $q);
		$this->db->limit('20');
		$result = $this->db->get('saran');
		return $result->result_array();
	}
