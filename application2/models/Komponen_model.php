...
class Komponen_model extends CI_Model
{
  ...
  function get_data($where = array())
	...
	public function json_all_komponen($where, $limit, $start, $fields, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
  ...
  function simpan_komponen($data_input, $table_name)
	{
		$this->db->insert( $table_name, $data_input );
		return $this->db->insert_id();
	}
		
  function update_data_komponen($data_update, $where, $table_name)
	{
		$this->db->where($where);
		$this->db->update($table_name, $data_update);
	}
	
	function json_semua_komponen($where) {	
		$this->db->from('komponen');
		$this->db->where($where);
	...
	function count_all_search_komponen($where, $key_word, $table_name, $field) {	
		$this->db->from($table_name);
		$this->db->where($where);
	...
	public function search_komponen($fields, $where, $limit, $start, $key_word, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
		$this->db->like('urut_komponen', $key_word);
		$this->db->order_by($order_by);
		$this->db->limit($limit, $start);
		$result = $this->db->get('komponen');
		return $result->result_array();
	...
	function auto_suggest($q, $where, $fields) {
		$this->db->select("$fields");
		$this->db->where($where);
		$this->db->like('komponen.urut_komponen', $q);
		$this->db->or_like('komponen.judul_komponen', $q);
		$this->db->limit('20');
		$result = $this->db->get('komponen');
		return $result->result_array();
	}
