...
class Pengunjung_model extends CI_Model
{
	...
	public function json_all_halaman_menu($where, $limit, $start, $fields, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
		...
		$result = $this->db->get('halaman_menu');
  ...
  function simpan_halaman_menu($data_input, $table_name)
	{
		$this->db->insert( $table_name, $data_input );
		return $this->db->insert_id();
	}
		
  function update_data_halaman_menu($data_update, $where, $table_name)
	{
		$this->db->where($where);
		$this->db->update($table_name, $data_update);
	}
	
	function json_semua_halaman_menu($where) {	
		$this->db->from('halaman_menu');
		$this->db->where($where);
	...
	function count_all_search_halaman_menu($where, $key_word, $table_name, $field) {	
		$this->db->from($table_name);
		$this->db->where($where);
	...
	public function search_halaman_menu($fields, $where, $limit, $start, $key_word, $order_by) {
		$this->db->select("
		$fields
		");
    $this->db->where($where);
		$this->db->like('urut_halaman_menu', $key_word);
		$this->db->order_by($order_by);
		$this->db->limit($limit, $start);
		$result = $this->db->get('halaman_menu');
		return $result->result_array();
	...
	function auto_suggest($q, $where, $fields) {
		$this->db->select("$fields");
		$this->db->where($where);
		$this->db->like('halaman_menu.urut_halaman_menu', $q);
		$this->db->or_like('halaman_menu.judul_halaman_menu', $q);
		$this->db->limit('20');
		$result = $this->db->get('halaman_menu');
		return $result->result_array();
	}
