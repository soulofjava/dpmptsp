...
if(!function_exists('gavern_admin_menu')) {
	...
	function gavern_admin_menu() {		
		...
		if(get_option($gk_tpl->name . "_branding_admin_page_image") != '') {
			$icon_path = get_option($gk_tpl->name . "_branding_admin_page_image");
		}
		// creating main menu item for the template settings
		$plugin_page = add_menu_page(
			'GavernWP Framework', 
			$gk_tpl->config['template']->name, 
			'manage_options',
			'gavern-menu', 
		...
		if($gk_tpl->config['developer_config']->visibility->template_options == 'true') {
			//
		...
		if($gk_tpl->config['developer_config']->visibility->importexport == 'true') {
			//
			$plugin_page = add_submenu_page( 
				'gavern-menu', 
				$gk_tpl->config['template']->name, 
		...
		if($gk_tpl->config['developer_config']->visibility->updates == 'true') {
			//
			$plugin_page = add_submenu_page( 
				'gavern-menu', 
				$gk_tpl->config['template']->name, 
		...
		if($gk_tpl->config['developer_config']->visibility->documentation == 'true') {
			//
			$plugin_page = add_submenu_page( 
				'gavern-menu', 
				$gk_tpl->config['template']->name, 
		...
		if($gk_tpl->config['developer_config']->visibility->gavickpro_website == 'true') {
			//
			$plugin_page = add_submenu_page( 
				'gavern-menu', 
				$gk_tpl->config['template']->name, 
...
if(!function_exists('gavern_widgets_init')) {
	...
	function gavern_widgets_init() {
		...
		$json_data = $gk_tpl->get_json('config','widgets');
		// init the widgets array
		$gk_tpl->widgets = array();
		...
		foreach ($json_data as $widget_area) {
			...
			if(isset($widget_area->amount)) {
				$gk_tpl->widgets[(string) $widget_area->id] = (int) $widget_area->amount;
			}
			// register sidebar
			register_sidebar(
				array(
