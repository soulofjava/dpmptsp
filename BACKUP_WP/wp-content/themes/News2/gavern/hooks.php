...
function gavernwp_body_attributes_hook() {
 	...
 	if(is_home()) {
 		...
 		if(get_option($gk_tpl->name . '_background_image', 'Y') == 'N') {
 			$classes .= ' no-bg';
 		}
 	...
 	echo ' data-tablet-width="'. get_option($gk_tpl->name . '_tablet_width', 1100) .'"';
 	
 	// generate the table-width attribute
 	echo ' data-tablet-small-width="'. get_option($gk_tpl->name . '_small_tablet_width', 840) .'"';
 	
 	// generate the mobile-width attribute
 	echo ' data-mobile-width="'. get_option($gk_tpl->name . '_mobile_width', 640) .'"';
...
function gavernwp_ga_code_hook() {
	...
	if(get_option($gk_tpl->name . '_ga_ua_id', '') != '') {
		...
		<?php if(get_option($gk_tpl->name . '_ga_ua_type', 'ga') == 'ga' ) : ?>
		...
		<?php else : ?>
			<script>
			  ...
			  ga('create', '<?php echo get_option($gk_tpl->name . '_ga_ua_id', ''); ?>', 'auto');
			  ga('send', 'pageview');
