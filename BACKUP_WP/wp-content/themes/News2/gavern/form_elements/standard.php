...
class GKFormInput {
	 ...
	 *
	 **/
	function __construct($gk_tpl, $name, $label, $tooltip = '', $default = '', $class = '', $format = '', $required = false, $visibility = '', $other = null) {
...
/**
 *
 * RowColumn - used to specify amount of columns/rows
 *
 **/

class GKFormInputRowColumn extends GKFormInput {
	public function getValue($default) {
		// get the option value from database or if it doesn't exist get the default value
		$this->value = array(
			"column" => get_option($this->tpl->name . "_" . str_replace('_row', '', $this->name), $default),
			"row" => get_option($this->tpl->name . "_" . str_replace('_column', '', $this->name), $default)
		);
	}
	...
	public function output() {	
		$output = '<p data-visible="true"><label 
			...
			<input 
				...
				size="'.($this->other->size).'" 
				min="'.($this->other->min).'" 
				max="'.($this->other->max_first).'" 
				step="'.($this->other->step).'"
				class="gkInput gkWidthHeight"
				id="'.($this->tpl->name . "_" . str_replace('_column', '', $this->name)).'" 
				name="'.($this->tpl->name . "_" . str_replace('_column', '', $this->name)).'" 
			...
			<input 
				type="number" 
				min="'.($this->other->min).'" 
				max="'.($this->other->max_second).'" 
				step="'.($this->other->step).'"
				class="gkInput gkWidthHeight"
				size="'.($this->other->size).'" 
				id="'.($this->tpl->name . "_" . str_replace('_row', '', $this->name)).'" 
				name="'.($this->tpl->name . "_" . str_replace('_row', '', $this->name)).'" 
