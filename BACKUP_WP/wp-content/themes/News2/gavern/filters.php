...
function gavern_widget_title_icon($title) {
	if($title == '&nbsp;' || trim($title) == '' || strlen($title) == 0) {
		...
		//
		$title = preg_replace('@\[(.*?)\]@', '<i class ="fa $1"></i>', $title);
		//
		return $title;
	}
}
...
function gavern_post_aside_class($classes) {
	...
	if($social_api_output != '' ) {
		$classes[] .= 'gk-has-aside';
	}
	//
	return $classes;
}
...
/**
 *
 * Function used to add featured-image class
 *
 * @return the modified list of the post classes
 *
 **/
function add_featured_image_class($classes) {
	...
	if ( has_post_thumbnail() && $params_image == 'Y' ) {
		array_push($classes, 'has-featured-image');
	}
	return $classes;
}

add_action('post_class', 'add_featured_image_class' );
