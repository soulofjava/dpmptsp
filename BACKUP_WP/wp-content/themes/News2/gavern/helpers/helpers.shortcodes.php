...
if(get_option($gk_tpl->name . '_shortcodes2_state', 'Y') == 'Y') {  	  
	...
	if(!function_exists('gavern_ies_related_posts')) {
		...
		function gavern_ies_related_posts($atts) {
			...
			//
			global $gk_tpl, $wpdb, $post, $table_prefix;
...
if(get_option($gk_tpl->name . '_shortcodes3_state', 'Y') == 'Y') {  
	...
	if(!function_exists('gk_last_modifications')) {
		// last modification posts/pages
		// Usage: 
		// [last_modified]
		function gk_last_modifications($atts) {
			global $wpdb;
			$last_modified = $wpdb->get_var("SELECT MAX($wpdb->posts.post_modified) FROM $wpdb->posts WHERE $wpdb->posts.post_status = 'publish'");
			return date('g:i A T, M j, Y', strtotime($last_modified));
		}
		
		add_shortcode('last_modified', 'gk_last_modifications');
	}
	
	
	if(!function_exists('gk_like_box')) {
		// likebox, usage: 
		// [fb_like_box], [fb_like_box width="200" height="300" url="URL" color="dark"]
		function gk_like_box($atts) {
			global $gk_tpl;
			// extract the attributes
			extract(
				shortcode_atts(
			...
			// variable to store the code output
			$output = '';
			
			$output .= '<div class="fb-like-box" data-href="'. $url .'" data-colorscheme="'. $color .'" data-height="'. $height .'" data-width="'. $width .'" data-show-faces="'. $faces .'" data-header="'. $show_header .'" data-stream="'. $stream .'" data-show-border="'. $border .'"></div>';
			
			// return the code
			return $output;
		}
		
		add_shortcode('fb_like_box', 'gk_like_box');
	}
}  
