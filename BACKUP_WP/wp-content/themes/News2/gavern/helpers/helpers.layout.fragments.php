 	...
 	?> 
	 	...
	 	<ul class="gk-meta-post">
	 		...
	 		<?php if(get_post_format() != '') : ?>
	 			<li class="format gk-format-<?php echo get_post_format(); ?>"></li>
	 		<?php endif; ?>
	 		...
	 		<?php endif; ?>
	 			...
	 			<?php if($param_author && !is_archive() && !$gk_tpl->isLatestPosts && !is_home()) : ?>
	 			<li>
	 			...
	 			<?php if((is_search()) || (get_post_type() != 'page' && $param_category) && !is_attachment() && !$gk_tpl->isLatestPosts && !is_category()) : ?>
	 			<li class="category">
	 			...
	 			<?php if ( comments_open() && ! post_password_required() && $param_comments && !is_tag()) : ?>
	 			<li class="gk-comments">
	 				<?php if (is_archive() || is_author() || is_category() || is_tag() || is_search() || $gk_tpl->isLatestPosts || is_home()) : ?>
		 				<?php 
		 					echo '<i class="fa fa-comments gk-comments"></i>';
		 					comments_popup_link(
		 			...
		 			<?php else : ?>
	 					<?php 
	 						comments_popup_link(
	 							'<span class="leave-reply">' . __( '0', GKTPLNAME ) . '</span>', 
	 							__( '<span>1</span> comment', GKTPLNAME ), 
	 							__( '<span>%</span> comments', GKTPLNAME )
	 						);
	 					?>
	 				<?php endif; ?>
	 			</li>
...
function gk_social_api($title, $postID) {
	...
	if($social_api_mode != 'none' && $is_excluded) {
		...
		$output = '<section id="gk-social-api">' . apply_filters('gavern_social_api_twitter', $twitter_output) . apply_filters('gavern_social_api_fb', $fb_like_output) . apply_filters('gavern_social_api_gplus', $gplus_output) . apply_filters('gavern_social_api_pinterest', $pinterest_output) . '</section>';
